...
const {
  ...
  setVoiceMoment
} = require("../../utils/route-engine/voice-schema");
const { adaptPublishedRoute } = require("../../utils/route-engine/elder-route-adapter");
  ...
  return {
    ...
    steps: route.steps.map((step) => {
      ...
      const transitWarning =
        step.type === "BUS_ON" && transit && !transit.direction
          ? "百度未提供公交行驶方向，请家属现场确认后再发布。"
          : (step.type === "SUBWAY_IN" || step.type === "SUBWAY_OUT") &&
              transit &&
              !transit.accessName
            ? `百度未提供${step.type === "SUBWAY_IN" ? "进站口" : "出站口"}，请家属现场确认并补充照片。`
            : "";
